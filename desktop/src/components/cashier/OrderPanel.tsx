...
import { IconTrash, IconShoppingCart, IconPlayerPause } from '@tabler/icons-react';
    ...
    >
      <div
        style={{
          ...
          borderRadius: 16, width: '100%', maxWidth: 420, margin: '0 16px', overflow: 'hidden',
        }}
      >
        <div className="flex flex-col items-center gap-1 px-6 pt-6 pb-4">
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#ffffff', letterSpacing: '-0.01em' }}>Confirm Order?</h2>
          {nextNumber === null ? (
            <span style={{ fontSize: 24, fontWeight: 800, color: '#404040', letterSpacing: '0.02em' }}>...</span>
          ) : (
            <span style={{ fontSize: 24, fontWeight: 800, color: 'var(--accent-color, #C0392B)', letterSpacing: '0.02em' }}>{nextNumber}</span>
          )}
        </div>

        <div style={{ height: 1, backgroundColor: '#2C2C2C' }} />

        <div className="hide-scrollbar" style={{ maxHeight: 200, overflowY: 'auto', padding: '0 24px' }}>
          {items.map((item, i) => (
            <div key={item.menu_item_id}>
              <div className="flex items-center gap-3 py-2">
                <span style={{ minWidth: 24, height: 24, borderRadius: 6, backgroundColor: 'var(--accent-color, #C0392B)', color: '#ffffff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, padding: '0 5px' }}>
                  {item.quantity}×
                </span>
                <span style={{ flex: 1, fontSize: 13, color: '#ffffff' }}>{item.item_name}</span>
                <span style={{ fontSize: 13, color: '#ffffff', fontWeight: 500 }}>{formatCurrency(item.item_price * item.quantity)}</span>
              </div>
              {i < items.length - 1 && <div style={{ height: 1, backgroundColor: '#242424' }} />}
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <span style={{ fontSize: 11, color: '#606060', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600 }}>Total</span>
          <span style={{ fontSize: 22, fontWeight: 800, color: 'var(--accent-color, #C0392B)' }}>{formatCurrency(totalAmount)}</span>
        </div>

        <div className="flex flex-col gap-2 px-6 pb-6">
          <button onClick={onConfirm} disabled={loading} style={{ width: '100%', padding: '12px', borderRadius: 10, border: 'none', backgroundColor: loading ? '#2C2C2C' : 'var(--accent-color, #C0392B)', color: loading ? '#606060' : '#ffffff', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', transition: 'opacity 0.15s' }}>
            {loading ? 'Placing Order...' : 'Place Order'}
          </button>
          <button onClick={onCancel} disabled={loading} style={{ width: '100%', padding: '12px', borderRadius: 10, border: '1px solid #2C2C2C', backgroundColor: '#111111', color: '#A0A0A0', fontSize: 14, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer' }}>
            Cancel
          </button>
        </div>
      </div>
...
export default function OrderPanel({ width }: { width: number }) {
  ...
  return (
    <div
      ...
      style={{ width, flexShrink: 0, display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#111111', borderLeft: '1px solid #2C2C2C' }}
    >
      {/* ── Header ── */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {cartItems.length > 0 && (
            <span style={{ backgroundColor: 'var(--accent-color, #C0392B)', color: '#ffffff', fontSize: 10, fontWeight: 700, borderRadius: 99, padding: '1px 7px', lineHeight: '16px' }}>
              {cartItems.length}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            style={{
              backgroundColor: heldOrders.length > 0 ? 'rgba(243,156,18,0.1)' : 'transparent',
              border: `1px solid ${heldOrders.length > 0 ? 'rgba(243,156,18,0.4)' : '#2C2C2C'}`,
              borderRadius: 6, padding: '4px 8px',
              color: heldOrders.length > 0 ? '#F39C12' : '#606060',
      ...
      {/* ── Footer ── */}
      {cartItems.length > 0 && (
        <div className="flex flex-col gap-3" style={{ padding: '14px 16px', borderTop: '1px solid #2C2C2C', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {paymentMethods.map(m => (
              <button
                ...
                style={{ flex: 1, minWidth: 70, height: 34, padding: '4px', borderRadius: 8, border: paymentMethod === m.name ? 'none' : '1px solid #2C2C2C', backgroundColor: paymentMethod === m.name ? 'var(--accent-color, #C0392B)' : '#1A1A1A', color: paymentMethod === m.name ? '#ffffff' : '#606060', fontSize: 11, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', cursor: 'pointer', transition: 'background-color 0.15s, color 0.15s' }}
              >
                {m.name}
              </button>
            ))}
          ...
          {isCash && (
            <div className="flex flex-col gap-1.5">
              <label style={{ fontSize: 11, color: '#606060', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Cash Tendered</label>
              <input
                type="number" placeholder="Enter amount" value={cashTendered}
                onChange={e => setCashTendered(e.target.value)}
                style={{ width: '100%', backgroundColor: '#1A1A1A', border: '1px solid #2C2C2C', borderRadius: 8, padding: '8px 12px', color: '#ffffff', fontSize: 13, outline: 'none' }}
          ...
          <button
            ...
            style={{ width: '100%', height: 48, borderRadius: 8, border: 'none', backgroundColor: loading ? '#2C2C2C' : 'var(--accent-color, #C0392B)', color: loading ? '#606060' : '#ffffff', fontSize: 15, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 15px rgba(192,57,43,0.3)', transition: 'opacity 0.15s' }}
          >
